...
import static com.lj.common.wxpay.WXPayConstants.USER_AGENT;
...
public class WXPayRequest {
    ...
    private String request(String urlSuffix, String uuid, String data, int connectTimeoutMs, int readTimeoutMs, boolean useCert, boolean autoReport) throws Exception {
        ...
        try {
            ...
            firstHasDnsErr = true;
            elapsedTimeMillis = WXPayUtil.getCurrentTimestampMs()-startTimestampMs;
            WXPayUtil.getLogger().warn("UnknownHostException for domainInfo {}", config.getWXPayDomain());
            WXPayReport.getInstance(config).report(
                    uuid,
                    elapsedTimeMillis,
                    config.getWXPayDomain(),
            ...
            elapsedTimeMillis = WXPayUtil.getCurrentTimestampMs()-startTimestampMs;
            WXPayUtil.getLogger().warn("connect timeout happened for domainInfo {}", config.getWXPayDomain());
            WXPayReport.getInstance(config).report(
                    uuid,
                    elapsedTimeMillis,
                    config.getWXPayDomain(),
            ...
            elapsedTimeMillis = WXPayUtil.getCurrentTimestampMs()-startTimestampMs;
            WXPayUtil.getLogger().warn("timeout happened for domainInfo {}", config.getWXPayDomain());
            WXPayReport.getInstance(config).report(
                    uuid,
                    elapsedTimeMillis,
                    config.getWXPayDomain(),
